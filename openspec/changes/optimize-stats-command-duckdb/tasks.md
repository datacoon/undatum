...
- [ ] 2.5.7 Handle edge cases: empty files, single-column files, very wide files
...
### 4.1 Unit Tests
- [ ] 4.1.1 Test `_detect_engine()` with various file types (CSV, JSONL, JSON, Parquet, XML, BSON)
- [ ] 4.1.2 Test `_detect_engine()` with compressed files (gzip, zstd)
- [ ] 4.1.3 Test `_detect_engine()` with explicit engine parameter
- [ ] 4.1.4 Test `_compute_duckdb_basic_stats()` with flat CSV file
- [ ] 4.1.5 Test `_compute_duckdb_basic_stats()` with nested JSON file
- [ ] 4.1.6 Test `_compute_duckdb_length_stats()` with various field types
- [ ] 4.1.7 Test `_detect_types_from_sample()` matches iterable engine type detection
- [ ] 4.1.8 Test `_compute_duckdb_dictionaries()` builds correct frequency dictionaries

### 4.2 Integration Tests
- [ ] 4.2.1 Test full `_stats_duckdb()` workflow with small CSV file (< 1000 rows)
- [ ] 4.2.2 Test full `_stats_duckdb()` workflow with large CSV file (> 100K rows)
- [ ] 4.2.3 Test full `_stats_duckdb()` workflow with JSONL file
- [ ] 4.2.4 Test full `_stats_duckdb()` workflow with nested JSON file
- [ ] 4.2.5 Test engine selection with `--engine auto` on supported format
- [ ] 4.2.6 Test engine selection with `--engine auto` on unsupported format
- [ ] 4.2.7 Test explicit `--engine duckdb` on supported format
- [ ] 4.2.8 Test explicit `--engine iterable` on supported format
- [ ] 4.2.9 Test fallback behavior when DuckDB query fails
- [ ] 4.2.10 Compare statistics output between DuckDB and iterable engines (should match)

### 4.3 Performance Tests
- [ ] 4.3.1 Benchmark DuckDB engine vs iterable engine on 10K row CSV file
- [ ] 4.3.2 Benchmark DuckDB engine vs iterable engine on 100K row CSV file
- [ ] 4.3.3 Benchmark DuckDB engine vs iterable engine on 1M row CSV file
- [ ] 4.3.4 Benchmark DuckDB engine vs iterable engine on JSONL files (various sizes)
- [ ] 4.3.5 Verify performance improvements (expect 10-100x speedup for large files)

### 4.4 Edge Cases
- [ ] 4.4.1 Test with empty file
- [ ] 4.4.2 Test with single-row file
- [ ] 4.4.3 Test with single-column file
- [ ] 4.4.4 Test with file containing all NULL values in a column
- [ ] 4.4.5 Test with file containing very long string values
- [ ] 4.4.6 Test with deeply nested JSON structures (4+ levels)
- [ ] 4.4.7 Test with malformed file that causes DuckDB error (verify fallback)

## 5. Documentation

### 5.1 Code Documentation
- [ ] 5.1.1 Add docstrings to all new methods (`_detect_engine`, `_stats_duckdb`, etc.)
- [ ] 5.1.2 Document engine selection logic and supported formats
- [ ] 5.1.3 Document fallback behavior and error handling

### 5.2 User Documentation
- [ ] 5.2.1 Update CLI help text for `--engine` option
- [ ] 5.2.2 Update README.md with DuckDB engine information
- [ ] 5.2.3 Add examples showing `--engine` option usage
- [ ] 5.2.4 Document performance benefits of DuckDB engine
- [ ] 5.2.5 Document which formats support DuckDB engine

## 6. Validation

### 6.1 OpenSpec Validation
- [ ] 6.1.1 Run `openspec validate optimize-stats-command-duckdb --strict`
- [ ] 6.1.2 Fix any validation errors or warnings
- [ ] 6.1.3 Ensure all scenarios are properly formatted

### 6.2 Code Quality
- [ ] 6.2.1 Run linter (flake8/pylint) on modified files
- [ ] 6.2.2 Fix any linting errors
- [ ] 6.2.3 Ensure code follows existing patterns (match `counter.py`, `analyzer.py`)

### 6.3 Regression Testing
- [ ] 6.3.1 Run existing stats command tests to ensure no regressions
- [ ] 6.3.2 Verify iterable engine path still works correctly
- [ ] 6.3.3 Verify backward compatibility (default behavior unchanged)
