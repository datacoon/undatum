...
- [ ] 3.1 Add WHERE clause support to `get_duckdb_fields_freq()` function in `selector.py` - Deferred
- [ ] 3.2 Add WHERE clause support to `get_duckdb_fields_uniq()` function in `selector.py` - Deferred
- [x] 3.3 Create helper function to translate filter expressions to SQL WHERE clauses (basic support) - Placeholder created
- [ ] 3.4 Update `frequency()` method to pass filter to DuckDB functions - Deferred
- [ ] 3.5 Document limitations of DuckDB WHERE clause translation (which expressions are supported) - Deferred
- [ ] 3.6 Test DuckDB filtering with sample expressions - Deferred
...
- [ ] 7.4 Test `IN` and `CONTAINS` operations if supported - Requires mistql testing
- [ ] 7.5 Test `LIKE` pattern matching if supported - Requires mistql testing
...
- [ ] 7.8 Test DuckDB engine paths with filters - Deferred (WHERE clause support not yet implemented)
- [x] 7.9 Test iterable engine paths with filters
- [ ] 7.10 Performance testing: compare mistql vs dictquery performance with large datasets - Optional
- [x] 7.11 Test error handling for invalid filter expressions
- [ ] 7.12 Test backward compatibility with existing filter expressions from documentation/examples - Note: syntax differs (&& vs AND)

## 8. Documentation Updates

- [ ] 8.1 Update `README.md` to document filter expression syntax (if changed from dictquery) - Recommended
- [x] 8.2 Update `openspec/project.md` to remove dictquery from tech stack
- [ ] 8.3 Create migration guide if filter syntax differs from dictquery - Recommended (syntax differs)
- [ ] 8.4 Document filter expression examples in relevant command documentation - Recommended
...
- [ ] 9.5 Review `DICTQUERY_REMOVAL_REVIEW.md` and mark items as completed - Optional
