...
def randomize_email(value: Any) -> str:
    ...
    if len(parts) == 2:
        ...
        random_domain = randomize_string(domain.split('.')[0], length=min(len(domain.split('.')[0]), 8))
...
def randomize_number(value: Any, min_val: Optional[float] = None, max_val: Optional[float] = None) -> Any:
...
def mask_value(value: Any, method: str, field_name: Optional[str] = None, salt: Optional[str] = None) -> Any:
