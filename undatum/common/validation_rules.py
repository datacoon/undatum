...
class ValidationRule:
    ...
    def _evaluate_field_rule(
        ...
        if "custom" in self.rule_def:
            ...
            if custom_func_name in VALIDATION_RULEMAP:
                ...
                logger.warning(
                    f"Custom validation function '{custom_func_name}' not found in VALIDATION_RULEMAP"
                )
    ...
    def _evaluate_cross_field_rule(
        ...
        try:
            ...
            allowed_names = {"None": None, "True": True, "False": False, **safe_vars}
    ...
    def _validate_format(self, value: Any, format_type: str) -> tuple[bool, Optional[str]]:
        ...
        if format_type in format_validators:
            if format_type in VALIDATION_RULEMAP:
                ...
                val_func = VALIDATION_RULEMAP.get(
                    f"common.{format_type}"
                ) or VALIDATION_RULEMAP.get(format_type)
                if val_func:
                    if val_func(value):
                        return True, None
                    else:
