...
class DatabaseQueryExecutor:
    ...
    def query(self, query: str, db_uri: str, output: Optional[str] = None, 
              output_format: str = 'jsonl', batch_size: int = 10000):
        ...
        try:
            ...
            if db_type == 'postgresql':
                try:
                    ...
                    cursor = conn.cursor(name='undatum_cursor', cursor_factory=psycopg2.extras.RealDictCursor)
                except Exception:
            ...
            try:
                ...
                if output_format == 'jsonl':
                    ...
                    columns = self._output_parquet(cursor, columns, output or 'output.parquet', batch_size)
                else:
        ...
        while True:
            ...
            for row in rows:
                if isinstance(row, dict):
                    ...
                    if not columns:
                        columns = [f'column_{i}' for i in range(len(row))]
                    all_rows.append(dict(zip(columns, row) if len(columns) == len(row) else {f'column_{i}': val for i, val in enumerate(row)}))
