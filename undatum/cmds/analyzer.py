...
def analyze(
    ...
    report = ReportSchema(
        filename=filename, file_size=filesize, file_type=filetype, compression=compression
    )
    ...
    if filetype in TEXT_DATA_TYPES:
        if encoding is None:
            ...
            report.metadata["encoding"] = (
                encoding_result.get("encoding") if encoding_result else None
            )
        else:
